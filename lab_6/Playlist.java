...
import lab_6.exceptions.InvalidTrackDataException;
...
public class Playlist implements MusicCollection, Serializable {
    ...
    public void write(Writer out) throws IOException {
        String safeTitle = (playlistTitle != null ? playlistTitle : "Unknown").replace(" ", "_");
        out.write(safeTitle + " ");
        out.write(genreId + " ");
    ...
    /**
     * Задание 1: реализация метода compareTo() для сортировки по возрастанию
     * метрики
     */
    @Override
    public int compareTo(MusicCollection other) {
        try {
            double thisMetric = this.calculateMetric();
            double otherMetric = other.calculateMetric();
            return Double.compare(thisMetric, otherMetric);
    ...
    /**
     * Задание 3: реализация метода iterator() из интерфейса Iterable
     */
    @Override
    ...
    /**
     * Задание 3: внутренний класс итератора для обхода элементов массива
     */
    private class PlaylistIterator implements Iterator<Integer> {
        private int currentIndex = 0;

        @Override
        public boolean hasNext() {
            return currentIndex < trackRatings.length;
        }

        @Override
        public Integer next() {
            return trackRatings[currentIndex++];
        }
    }
